# ============================================================
# IAM - Roles for EC2 (SSM + S3 read) and Lambda (ALB describe)
# ============================================================
...
resource "aws_iam_role_policy" "lambda_describe" {
  name = "${var.project_name}-lambda-describe"
  role = aws_iam_role.lambda.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect   = "Allow"
      Action   = ["elasticloadbalancing:DescribeTargetHealth", "cloudwatch:PutMetricData"]
      Resource = "*"
    }]
  })
}
